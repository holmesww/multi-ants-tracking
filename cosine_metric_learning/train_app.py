...
def create_default_argument_parser(dataset_name):
	...
	parser.add_argument(
		"--batch_size", help="Training batch size", default=16, type=int)
	parser.add_argument(
	...
	parser.add_argument(
		"--number_of_steps", help="Number of train/eval steps. If None given, "
								  "runs indefenitely", default=100000, type=int)
	parser.add_argument(
...
def train_loop(preprocess_fn, network_factory, train_x, train_y,
			   ...
			   number_of_steps=100000, loss_mode="cosine-softmax",
			   learning_rate=1e-4, trainable_scopes=None,
			   save_summaries_secs=10, save_interval_secs=10):
	"""Start training.
...
def finalize(preprocess_fn, network_factory, checkpoint_path, image_shape, output_filename):
	...
	with tf.Session(graph=tf.Graph(),
					config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as session:
