...
import train_app
from datasets import ants
from datasets import util
import nets.ant_tracking.network_definition as net
...
class Ants(object):
    ...
    def __init__(self, dataset_dir, num_validation_y=0, seed=1234):
        self._dataset_dir = dataset_dir
        self._num_validation_y = num_validation_y
        self._seed = seed

    def read_train(self):
        filenames, ids, camera_indices, _ = ants.read_train_split_to_str(
            self._dataset_dir)
...
def main():
    arg_parser = train_app.create_default_argument_parser("ants")
    arg_parser.add_argument(
        "--dataset_dir", help="Path to ANTS dataset directory.",
        default="resources/AntTracking")
    args = arg_parser.parse_args()
    dataset = Ants(args.dataset_dir, num_validation_y=0, seed=1234)
    if args.mode == "train":
        ...
        network_factory = net.create_network_factory(
            is_training=True, num_classes=ants.MAX_LABEL + 1,
        ...
        network_factory = net.create_network_factory(
            is_training=False, num_classes=ants.MAX_LABEL + 1,
            add_logits=args.loss_mode == "cosine-softmax")
